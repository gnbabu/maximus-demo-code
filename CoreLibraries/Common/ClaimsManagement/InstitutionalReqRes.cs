...
{
    public class InstitutionalReqRes : ClaimReqRes
    {
        public InstitutionalReqRes() : base()
        {
            SetServieConfigData();
        }
        private Logging log;
        ...
        {
            url = AppSettings.Get("ClaimsInquireInstitutionalWSURL", Constants.WebServiceURI.ClaimsInstitutionalService);
            //url = "http://localhost:8091/ClaimsServiceInstitutionalSOAPVS";
            //url = "http://10.118.47.72:9103/ClaimsServiceInstitutionalSOAPVS";
        ...
        private string claimsInquireSoapAction = "http://service.operationmgmt.fi/ClaimsService/institutional/InquireClaim";
        /// <summary>
        /// Claims Institutional Inquiry service call
        /// </summary>
        ...
        public InquireClaimResponse InquireClaimRequest(string payorType, string icn, string providerId)
        {
            ...
            log.CreateLogEntry(string.Format("{0} {1}", " Institutional inquireClaimReq.MessageHeader", msgheader), Logging.LogPriority.Information);
            inquireClaimReq.MessageHeader = msgheader;
            inquireClaimReq.RequestPayload = reqPayLoadType;
            log.CreateLogEntry(string.Format("{0} {1}", "Institutional inquireClaimReq.RequestPayload", reqPayLoadType), Logging.LogPriority.Information);
            ...
            xml = "<soapenv:Envelope xmlns:soapenv=\"http://schemas.xmlsoap.org/soap/envelope/\" xmlns=\"http://service.operationmgmt.fi/ClaimsService/institutional\">"
            ...
            return GetClaimsInstitutionalInquireResponse(icn, providerId, xmlDoc, sitTransactionKey);
        }

        /// <summary>
        /// To get Claims Institutional Inquiry service response
        /// </summary>
        ...
        /// <returns></returns>
        private InquireClaimResponse GetClaimsInstitutionalInquireResponse(string icn, string medicaidId, XmlDocument xmlDoc, string sitTransactionKey)
        {
            int pid;
            string logHeader = string.Format("Get Claims Institutional Inquiry for ICN# - " + icn);
            ...
            log.CreateLogEntry(string.Format("{0} {1}", "Institutional Claim Inquiry Search request XML :", xmlDoc.InnerXml.ToString()), Logging.LogPriority.Information);
            ...
            //log.CreateLogEntry(string.Format("{0} {1}", "", "Institutional" + pmHttpRequest), Logging.LogPriority.Information);
            //log.CreateLogEntry(string.Format("{0} {1}", "Institutional Claim Inquiry Search request :", pmHttpRequest), Logging.LogPriority.Information);
            //log.CreateLogEntry(string.Format("{0} {1}", "Institutional Claim Inquiry Search request for claimsInquireSoapAction :", claimsInquireSoapAction), Logging.LogPriority.Information);
            ...
            //                      (int)CON.ClaimType.INSTITUTIONAL, "For ICN: " + icn, icn, null, null, null, null);
            ...
            if (requestStream != null || clDentalWebAPITesting.Equals("true"))
            {
                try
                {
                    ...
                    {
                        DataSet ds = RecipientEligibilityDA.RetrieveWebAPITestingResponse("ClaimsInquiryInstitutionalWebAPI");
                    ...
                    pid = PassthroughController.InsertPassthroughTransactionQueue((int)PassthroughTransactionType.ClaimInquiry, sitTransactionKey, null, null,
                               ...
                               (int)CON.ClaimType.INSTITUTIONAL, string.Empty, icn, null, null, null, null);
                }
                catch (Exception ex)
                {
                    pid = PassthroughController.InsertPassthroughTransactionQueue((int)PassthroughTransactionType.ClaimInquiry, sitTransactionKey, null, null,
                               xmlDoc.InnerXml.ToString(), null, "PNM", "FI", medicaidId, DateTime.Now, DateTime.Now, DateTime.Now, DateTime.Now, Methods.GetCurrentUserId().ToString(), Methods.GetCurrentUserId().ToString(),
                               (int)CON.ClaimType.INSTITUTIONAL, string.Empty, icn, null, null, ex.Message, null);
                }
                ...
                if (!response.Contains("<faultcode>"))
                {
                    ...
                    var serializerResp = new XmlSerializer(typeof(InstitutionalEnvRes));
                    InstitutionalEnvRes eir = (InstitutionalEnvRes)serializerResp.Deserialize(new StringReader(xDocResp.ToString()));
                    ...
                    if (resr != null)
                    {
                        ...
                        {
                            log = new Logging(m_threadId, "");
                            log.CreateLogEntry(string.Format("{0} {1}", "UpdateTransactionQueueFIResponse for Institutional", ex), Logging.LogPriority.Error);
                        }
                    }
                }
                else
                {
                    log.CreateLogEntry(string.Format("{0} {1}", "InstitutionalInquiryResponseError", response.ToString()), Logging.LogPriority.Error);
                    pid = PassthroughController.InsertPassthroughTransactionQueue((int)PassthroughTransactionType.ClaimInquiry, sitTransactionKey, null, null,
                               xmlDoc.InnerXml.ToString(), null, "PNM", "FI", medicaidId, DateTime.Now, DateTime.Now, DateTime.Now, DateTime.Now, Methods.GetCurrentUserId().ToString(), Methods.GetCurrentUserId().ToString(),
                               (int)CON.ClaimType.INSTITUTIONAL, string.Empty, icn, null, null, null, null);
