<%@ control language="C#" autoeventwireup="true" inherits="PopupControls_CertSecondGrid, App_Web_wbqq1lcm" %>
...
<asp:Panel ID="pnlCertificationsCLIA" runat="server">
    <div class="divGrid">
        <asp:GridView runat="server" Width="98%" ID="grdCertSecondGrid" AutoGenerateColumns="False" 
            HorizontalAlign="Left" CssClass="gridview" EmptyDataText="No CLIA number found" OnRowCommand="grd_RowCommand"
            AllowSorting="true" AllowPaging="false" PageSize="2" OnRowDataBound="grdCertSecondGrid_RowDataBound">
            <Columns>
                 <asp:TemplateField HeaderText="CLIA Number">
                    <ItemTemplate>
                       <asp:Label ID="lblCLIANo" runat="server"  Text='<%# Eval("CLIA_NUMBER") %>' />
                       <asp:LinkButton runat="server" ID="lnkBtnCLIA" Text='<%# Eval("CLIA_NUMBER") %>'  CommandName="ShowCliaLinkage" CommandArgument="<%# ((GridViewRow) Container).RowIndex %>"/>
                 </ItemTemplate>
                </asp:TemplateField>
                
                <asp:BoundField DataField="CLIA_EFF_DATE" HeaderText="Effective Date" SortExpression="CLIA_EFF_DATE" DataFormatString="{0:d}" Visible="false" />
                <asp:TemplateField HeaderText="Effective Date">
                    <ItemTemplate>
                        <%# Helper.FormatDate2(Eval("CLIA_EFF_DATE").ToString()) %>
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:BoundField DataField="CLIA_END_DATE" HeaderText="Expiration Date" SortExpression="CLIA_END_DATE" DataFormatString="{0:d}" Visible="false" />
                ...
                <asp:TemplateField ItemStyle-Width="2%" HeaderText="<span style='display:none'>Edit</span>">
                    <ItemTemplate>
                        <asp:ImageButton ID="btnEdit" alt="EditButton" runat="server" CommandName="EditCertSecondGridRow"
                            CommandArgument="<%# ((GridViewRow) Container).RowIndex %>" ImageUrl="~/Images/edit.png" ToolTip="Edit" />
                    </ItemTemplate>
                </asp:TemplateField>
                <asp:TemplateField HeaderText="<span style='display:none'>Delete</span>">
                    <ItemTemplate>
                        <asp:ImageButton ID="btnDelete" runat="server" CommandName="DeleteCertSecondGridRow" CommandArgument="<%# ((GridViewRow) Container).RowIndex %>"
