...
{
    public class WFDenyTermProvider : BaseWorkflowTask, IWorkflowTask
    {
        string nextStep = null;

        public WFDenyTermProvider(int processID, int stepID)
        ...
        override public bool ProcessTask()
        {
            ...
            try
            {
                ...
                if (ObjectControllerHelper.HasRows(ds))
                {
                    ...
                    if (!string.IsNullOrEmpty(ObjectControllerHelper.GetString("IMMEDIATE_DENY_OR_TERMINATE", dtProvider.Rows[0])))
                    {
                        if (ObjectControllerHelper.GetString("IMMEDIATE_DENY_OR_TERMINATE", dtProvider.Rows[0]) == CON.ImmediateDenyOrTerminate.Immediate.ToString())
                        {
                            nextStep = partyID > 0 ? "Immediate Term Request" : "Immediate Deny Request";
                            toReturn = true;
                        }
                        else
                        {
                            if (!string.IsNullOrEmpty(ObjectControllerHelper.GetString("APPEAL_STATUS_ID", dtProvider.Rows[0])))
                            {
                                ...
                                {
                                    nextStep = partyID > 0 ? "Terminate" : "Deny";
                                }
                                else if (AppealStatusID == CON.AppealStatus.NoticeSent || AppealStatusID == CON.AppealStatus.ProviderRequestedAppeal)
                                {
                                    ...
                                    {
                                        nextStep = "Appeal Process Site Visit";
                                    }
                                    else
                                    {
