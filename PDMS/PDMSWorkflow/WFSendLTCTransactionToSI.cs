...
{
    public class WFSendLTCTransactionToSI : BaseWorkflowTask, IWorkflowTask
    {
        string nextStep = null;

        public WFSendLTCTransactionToSI(int processID, int stepID)
        ...
        override public bool ProcessTask()
        {
            ...
            try
            {
                ...
                if (makeRequest.ToLower().Equals("true"))
                {
                    ...
                    if ((tId == (int)TransactionController.TransactionTypeNew.SendMMISEnroll) && (workflowEventTypeID == CON.WorkflowEventType.NewReg))
                    {
                    ...
                    if (transactionType > 0)
                    {
						...
						//RegistrationController.UpdateRegistrationAffiliationStatus(regID, CON.GroupAffiliationStatusTypeID.GroupConfirmed, DateTime.Now, new Guid(CON.appPDMSAdminUserId)); //OHPNM-12794
						...
						//RegistrationController.UpdateRegistrationAffiliationStatus(regID, CON.GroupAffiliationStatusTypeID.Active, DateTime.Now, new Guid(CON.appPDMSAdminUserId)); //OHPNM-12794
