...
{
    public class WFReceiveWaiverAcknowledgmentEnd : BaseWorkflowTask, IWorkflowTask
    {
        string nextStep = null;

        public WFReceiveWaiverAcknowledgmentEnd(int processID, int stepID)
        ...
        override public bool ProcessTask()
        {
            ...
            try
            {
                ...
                int enrollmentStatusCode = ObjectControllerHelper.GetInt("REG_SERVICE_LOCATION_ID", drReg);
                int waiverTypeID = ObjectControllerHelper.GetInt("WaiverTypeID", drReg);
                DateTime? termDate = null;
                DateTime tmp;
                if (DateTime.TryParse(Methods.GetStringValue(drReg["TerminationDate"]), out tmp))
                {
                    termDate = tmp;
                }
                ...
                if(nextStep == "Transaction Passed" && termDate.HasValue)
                {
                    List<SqlParameter> sqlParmsRS = new List<SqlParameter>();
                    sqlParmsRS.Add(SqlParms.CreateParameter("REG_ID", DbType.Int32, regID, false));
                    DataSet dsRegSpec = DataAccess.ExecuteStoredProcedure("usp_SelectREG_SPECIALTY", sqlParmsRS, "RegSpecData"); 
                    DataTable drRegSpeciality = ObjectControllerHelper.HasRows(dsRegSpec)
                   ? dsRegSpec.Tables[0].AsEnumerable()
                           .Where(row => (row.Field<DateTime>("END_DATE") > DateTime.Now)).CopyToDataTable()
                   : null;

                    bool HasActiveODASpecialty = false;
                    if (drRegSpeciality != null && drRegSpeciality.Rows.Count > 0)
                    {
                        HasActiveODASpecialty = drRegSpeciality.AsEnumerable()
                                                       .Where(x => (Convert.ToString(x["MMIS_SPECIALTY_TYPE_ID"]) == CON.MMISSpecialtyType.ODAWaiver)
                                                                    || (Convert.ToString(x["MMIS_SPECIALTY_TYPE_ID"]) == CON.MMISSpecialtyType.HCBSAssistedLiving))
                                                       .Count() > 0 ? true : false;
                    }

                    if (HasActiveODASpecialty)
                    {
