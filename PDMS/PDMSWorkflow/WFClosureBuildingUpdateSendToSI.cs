...
{
    class WFClosureBuildingUpdateSendToSI: BaseWorkflowTask, IWorkflowTask
    {
        string nextStep = null;

        public WFClosureBuildingUpdateSendToSI(int processID, int stepID)
        ...
        override public bool ProcessTask()
        {
            ...
            try
            {
                ...
                { 
                    ...
                    int tId = 0;
                    int BuildingRegId = RegistrationController.SelectBuildingRegistration(regID);                   
                   
                        transactionType = (int)TransactionController.TransactionTypeNew.SendMMISUpdate;
                        DataSet dsReg = RegistrationController.SelectSvcLocIDTranType(BuildingRegId);

                        if (transactionType > 0 && BuildingRegId > 0 && ObjectControllerHelper.HasRows(dsReg))
                        {
                           
                            mId = dsReg.Tables[0].Rows[0]["MEDICAID_ID"].ToString();
                            sId = Convert.ToInt32(dsReg.Tables[0].Rows[0]["REG_SERVICE_LOCATION_ID"]);
                            tId = Convert.ToInt32(dsReg.Tables[0].Rows[0]["TRANSACTION_TYPE_ID"]);
