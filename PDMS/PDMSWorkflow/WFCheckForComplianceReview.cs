...
{
    public class WFCheckForComplianceReview : BaseWorkflowTask, IWorkflowTask
    {
        string nextStep = null;
        string logMsg = String.Format(Constants.LogString.MethodSignature, MethodBase.GetCurrentMethod().DeclaringType.Name, MethodBase.GetCurrentMethod().Name);
        //Logging log = new Logging(LogThreadID, logMsg);

        public WFCheckForComplianceReview(int processID, int stepID)
        ...
        override public bool ProcessTask()
        {
            ...
            try
            {
                ...
                if (drReg != null)
                {
                    ...
                    bool checkIfComplainceReviewRequiredforPrimarySpecialtyESR = ObjectControllerHelper.GetBool("primarySpecialtyESR", drReg);

                    bool isRevalRegOfTerminatedProvider = isReactivation || isReapplication || 
                        (!ObjectControllerHelper.IsDateNull(oldtermDate) && (workflowEventTypeID == CON.WorkflowEventType.RevalReg || workflowEventTypeID == CON.WorkflowEventType.UpdateReg));

                    if (isRevalRegOfTerminatedProvider && checkIfComplainceReviewRequiredforPrimarySpecialtyESR)
                    {
                        nextStep = "Yes";
                        toReturn = true;
                    }
                    ...
                    {
                        if(workflowEventTypeID == CON.WorkflowEventType.UpdateReg && checkIfComplainceReviewRequiredforPrimarySpecialtyESR)
                        {
                            nextStep = "Yes";
                            toReturn = true;
                        }
