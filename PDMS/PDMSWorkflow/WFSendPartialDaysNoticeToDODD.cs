...
{
    public class WFSendPartialDaysNoticeToDODD : BaseWorkflowTask, IWorkflowTask
    {
        string nextStep = null;

        public WFSendPartialDaysNoticeToDODD(int processID, int stepID)
            ...
            try
            {
                ...
                if (canMakeWSRequest.Equals("true"))
                {
                    ...
                    if (transactionType > 0)
                    {
                        ...
                        RegistrationController.PopulateStagingData(tqId, CON.TransactionTypeValues.PSM_FULL, "ManualUpdate", CON.StageDocumentsTitle.FourtyFiveDaysNotice);



                        //CheckIfDocumentStaged(tqid, documentName)
                        ServicePointManager.Expect100Continue = true;
                        ServicePointManager.SecurityProtocol = SecurityProtocolType.Tls | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12 | SecurityProtocolType.Ssl3;

                        bool checkDocumentStaged = TransactionController.CheckIfDocumentStaged(tqId, CON.StageDocumentsTitle.FourtyFiveDaysNotice);
                        //Log value here
                        log.CreateLogEntry("RegId: " + regID + "; tqId: " + tqId + "; checkDocumentStaged: " + checkDocumentStaged, "WFSendPartialDaysNoticeToDODD");
                        if (checkDocumentStaged)
                        {
                            txnResult = pprr.partialProviderManagementSubmitRequest(tqId, Constants.PartialProviderSubscriberSystems.PSM, true, false);
                        }
                        else
                        {
                            txnResult = CON.TransactionResult.TransactionFailed;
                            TransactionController.UpdateTransactionQueueSIResponse(tqId, CON.ResponseCodes.PNM_Document_Failed, CON.ResponseTypes.PNM_Document_Failed, CON.ResponseMessage.PNM_Document_Failed, DateTime.Now, new Guid(CON.appAdminUserId));
                        }
