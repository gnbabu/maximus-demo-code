<%@ Control Language="C#" AutoEventWireup="true" Inherits="Views_RetroEffectiveView" Codebehind="RetroEffectiveView.ascx.cs" %>
...
<script type="text/javascript">
    ...
    function ValidateDates() {
        var effDateStr = $("input[id*='txtNewEffectiveDate']").val();
        var dtParts = effDateStr.split("/");
        if (dtParts.length == 3 && dtParts[2].length != 4) {
            document.getElementById("<%= lblErrorMessages.ClientID%>").innerHTML = "New Effective Date must be in format MM/DD/YYYY";
            return false;
        }
        var newRevalDateStr = $("input[id*='txtNewRevalDate']").val();
        var dtParts1 = newRevalDateStr.split("/");
        if (dtParts1.length == 3 && dtParts1[2].length != 4) {
            document.getElementById("<%= lblErrorMessages.ClientID%>").innerHTML = "New Revalidation Date must be in format MM/DD/YYYY";
            return false;
        }
    }
...
<div style="padding:5px;">
        ...
        <div class="row">
            <div class="col-sm-5 text-right">
                <span class="formLabel wd200">
                    New Revalidation Date
                    <!-- Dynamic asterisk - will be shown/hidden based on requirement -->
                    <asp:Label ID="lblRevalidationDateAsterisk" runat="server" Text="*" />
                </span>
            </div>
            <div class="col-sm-6 text-left">
                <asp:TextBox ID="txtNewRevalDate" OnChange="Verifydate()" runat="server" CssClass="formfieldDate" Style="height: 30px; width: 90px" MaxLength="10" />

                <ajax:calendarextender id="ceNewRevalDate" cssclass="QstCalendarCSS" format="MM/dd/yyyy" runat="server" targetcontrolid="txtNewRevalDate" runat="server" />

                <!-- RequiredFieldValidator - This will be enabled/disabled dynamically -->
                <asp:RequiredFieldValidator ID="valNewRevalDateReqd" runat="server" SetFocusOnError="true" ValidationGroup="RetroEffective" Text="*"
                    ControlToValidate="txtNewRevalDate" ErrorMessage="* New Revalidation Date is required." Display="Dynamic" Enabled="true" />

                <asp:CompareValidator ID="cvNewRevalDate" runat="server" ValidationGroup="RetroEffective"
                    Type="Date" Operator="DataTypeCheck" ControlToValidate="txtNewRevalDate" Enabled="true"
                    ErrorMessage="* Please enter a valid New Revalidation Date (mm/dd/yyyy)." Text="*" Display="Dynamic" ValueToCompare="MM/dd/yyyy"
                    SetFocusOnError="true"> 
        </asp:CompareValidator>
            </div>
        </div>
    ...
    <div class="btnBox" style="padding-top:10px;padding-right:10px;">
        <asp:Button ID="btnSave" runat="server" Text="Save" CssClass="buttonBoxFocus" OnClientClick="return ValidateDates(); return preventMultipleSubmissions();"  OnClick="btnSave_Click" CausesValidation="true" ValidationGroup="RetroEffective" />
