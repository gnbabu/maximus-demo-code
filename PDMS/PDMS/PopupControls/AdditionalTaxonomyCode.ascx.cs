...
public partial class PopupControls_AdditionalTaxonomyCode : System.Web.UI.UserControl
{
    ...
    private void SetDt()
    {
        if (dt == null)
        {
            DataSet ds = svc.SelectRegistrationData(this.WorkflowPage.RegistrationId, "ADDTL_TAXONOMY");
    ...
    public void LoadData(DataRow row, bool isEdit, string existingTaxos)
    {
        ...
        if (isEdit)
        {
            ...
            pdms_Code.Text = GetTaxonomyCode(row["PDMS_TAXONOMY_TYPE_ID"].ToString());
            pdms_Description.Text = GetTaxonomyName(row["PDMS_TAXONOMY_TYPE_ID"].ToString());
        }
        else
        {
    ...
    {
        Page.Validate("AdditionalTaxonomyCode");
        for (int i = 0; i < Page.Validators.Count; i++)
        {
            ...
            try
            {
                ...
                if (v != null && v.ValidationGroup.Equals("AdditionalTaxonomyCode") && !v.IsValid)
                    return false;
            }
            catch
            {
    ...
    protected void ValidateLC46_ERR23(object sender, ServerValidateEventArgs e)
    {
        e.IsValid = true;
        //SetDt();
    ...
    {
        e.IsValid = true;
        //if (!string.IsNullOrEmpty(prov_Code.SelectedValue.Trim()))
        //{
    ...
    protected void Validate_CodeRequired(object sender, ServerValidateEventArgs e)
    {
        e.IsValid = !string.IsNullOrEmpty(prov_Code.Text);
    }

    private string GetTaxonomyCode(string id)
    {
        int i;
        ...
        DataTable dt = TaxoCodes.Select("TAXONOMY_TYPE_ID = " + id).CopyToDataTable();
        if (Helper.HasRows(dt))
        {
            return dt.Rows[0]["TAXONOMY_CODE"].ToString();
        }
        else
        {
            return string.Empty;
    ...
    private string GetTaxonomyName(string id)
    {
        int i;
        if (!int.TryParse(id, out i))
        {
        ...
        if (Helper.HasRows(dt))
        {
            return dt.Rows[0]["TAXONOMY_NAME"].ToString();
        }
        else
        {
            return string.Empty;
