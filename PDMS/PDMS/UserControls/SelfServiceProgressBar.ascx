<%@ Control Language="C#" AutoEventWireup="true" Inherits="UserControls_SelfServiceProgressBar" Codebehind="SelfServiceProgressBar.ascx.cs" %>
        ...
        if (onLeave != 'true') {
            ...
            if (currentItem != "") {
                ...
                if (button.get_text() != toolbarbutton.get_text()) {
                    BeforeLeavingBarPA(args);
                }
            }
        }
    }

    function BeforeLeavingBarPA(args) {
        var authenticated = '<%=HttpContext.Current.User.Identity.IsAuthenticated%>';
        if (authenticated == 'True') {
            var storage = window.sessionStorage;
            var isSpecialityChanged = storage.getItem("isSpecialityChangedPA");
            if (isSpecialityChanged != null && isSpecialityChanged != undefined && isSpecialityChanged != '' && isSpecialityChanged == 'True') {
                $('#ctl00_MainContent_HasUnsavedDataPA').val('True');
            }
        
            //alert('HasUnsavedData: ' + $('#ctl00_MainContent_HasUnsavedDataPA').val());
            //alert('isProvAdmin:' + isProvAdmin);
            if ($('#ctl00_MainContent_HasUnsavedDataPA').val() == 'True') {
                //if (event) event.preventDefault ? event.preventDefault() : event.returnValue = false;
                //alert('isProvAdmin1:' + isProvAdmin);
                var dialogObj = $("#alertLeavePA");
                var dialog = dialogObj.dialog({
                    modal: true,
                    title: 'ALERT - Update not saved',
                    width: '600px',
                    buttons: {
                        'Ok': function () {

                            var lclStorage = window.sessionStorage;
                            lclStorage.removeItem("isSpecialityChangedPA");
                            ...
                            //Clear any Claims Data
                            if (typeof clearTables === "function" && clearTables.apply) {
                                clearTables();
                            }
                            if (typeof clrtbls === "function" && clrtbls.apply) {
                                clrtbls();
                            }
                            button.click();
                        },
...
<div id="DivsearchEliginilityProgressBar" class="searchEliginilityprogressbar">
        ...
        <div class="row">
            ...
            <div class="col-md-6 col-sm-6">
                <telerik:RadComboBox
                    ...
                    HighlightTemplatedItems="true" OnClientLoad="OnClientLoadHandler" OnClientItemsRequested="UpdateItemCountField" OnSelectedIndexChanged="RadJumpTo_SelectedIndexChanged" OnClientSelectedIndexChanged="OnClientSelectionChanged"
    ...
    <div id="divsearchEliginilityScroll" class="progressScroll">
        <telerik:RadToolBar ID="rrMenu" runat="server" OnButtonClick="rrMenu_ButtonClick" AutoPostBack="true" OnClientButtonClicking="OnClientButtonClicked"
    ...
    <br /> <br />
            <div class="row">
                <div class="col-lg-12 text-left">
                    ...
                    <asp:Label ID="lblProMedicaidID" class="formLabelAuto label-bold-700" Text="" runat="server" />      
                    <span class="formLabel200 label-bold-700">Provider Medicaid ID:<span></span>
                    </span>
                    ...
                    <span class="formLabel200 label-bold-700">Provider NPI:<span></span>
                    </span>
                    ...
                    <span class="formLabel200 label-bold-700">Provider Name:<span></span>
                    </span>
