...
{
    public class OnBaseDocUploadInterface : BaseJob, IJob
    {

        private int logCnt = 0;
        private Logging log = null;

        public OnBaseDocUploadInterface(Guid threadId)
            : base(threadId)
        {
            this.ThreadId = threadId;
        }

        private readonly string appID = "B2821231-91F9-4279-A8C4-705AA856F5FA";
        ...
        public void OnBaseDocUploadInitiate()
        {
            ...
            try
            {
                ...
                {
                    localPath = @"C:\temp\OHPNM\OnbaseTest\";
                    archiveFolder = @"C:\temp\OHPNM\OnbaseTest\Archive\";
                }
                                    ...
                                    if (docId > 0 && onbaseProcessRec1 != null && onbaseProcessRec1.Tables[0].Rows.Count == 1 && onbaseProcessRec1.Tables[0].Rows[0]["PROCESS_STATUS_CD"].ToString() == "P")
                                    {
                                        ...
                                        catch
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(returnId) && Convert.ToInt32(returnId) > 0)
                                                OnBaseDocUploadHelper.DeleteOnbaseUploadProcessById(log, new Guid(appID), Convert.ToInt32(returnId));
                                        }
                                    }
                                }
                                catch
