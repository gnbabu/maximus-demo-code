/****** Object:  StoredProcedure [dbo].[DCConv_ConvertGroups_Step1]    Script Date: 8/1/2016 3:13:13 PM ******/
IF object_id('[dbo].[DCConv_ConvertGroups_Step1]','P') is not null
...
--		BEGIN TRANSACTION
		...
		--SELECT DISTINCT en.[P-SYS-ID], ROW_NUMBER() OVER (ORDER BY en.[P-SYS-ID]) + @MaxRegistrationID, en.[P-STAT-EFF-DT], 1, NULL
		--FROM SRC_Enrollments en
		--INNER JOIN SRC_Providers prov ON prov.[P-SYS-ID] = en.[P-SYS-ID]
 ...
 */
		...
		WHERE map.IsGroup = 1 AND prov.[EnableConversion] = 1 and en.[EnableConversion] = 1;
		...
		-- obsolete
		--WITH taxAgg as (
		-- SELECT [P-SYS-ID], max([P-TAX-BEG-DT]) as [P-TAX-BEG-DT]
		-- FROM SRC_ProviderTaxIds 
		-- WHERE EnableConversion = 1
		-- GROUP BY [P-SYS-ID]		 
		--)
		--UPDATE REG_PROVIDER
		--SET [TAX_ID] = CASE WHEN RTRIM(prov.[P-SSN-NUM]) NOT IN ('','000000000') THEN prov.[P-SSN-NUM]
		--					WHEN reg.[ENTITY_TYPE_ID] <> 1 AND RTRIM(tax.[P-FED-TAX-ID]) NOT IN ('','000000000') THEN tax.[P-FED-TAX-ID]
		--					ELSE NULL END,
		--	[TAX_ID_TYPE_ID] = CASE WHEN RTRIM(prov.[P-SSN-NUM]) NOT IN ('','000000000') THEN 15
		--					WHEN reg.[ENTITY_TYPE_ID] <> 1 AND RTRIM(tax.[P-FED-TAX-ID]) NOT IN ('','000000000') THEN 16
		--					ELSE NULL END
		--FROM REG_PROVIDER reg
		--JOIN DCConv_KeyCrossReferences map ON reg.REG_ID = map.RegistrationID 
		--JOIN taxAgg on map.SysID = taxAgg.[P-SYS-ID]
		--JOIN SRC_ProviderTaxIds tax ON tax.[P-SYS-ID] = taxAgg.[P-SYS-ID] AND tax.[P-TAX-BEG-DT] = taxAgg.[P-TAX-BEG-DT]  
		--JOIN SRC_Providers prov ON prov.[P-SYS-ID] = map.SysID
		...
		--UPDATE REG_PROVIDER
		--SET [TAX_ID] = 
		--	CASE WHEN LEN(RTRIM(LTRIM(tax.[P-FED-TAX-ID]))) > 0 AND tax.[P-FED-TAX-ID] <> '000000000' THEN
		--		tax.[P-FED-TAX-ID]
		--	ELSE
		--		a.[P-SSN-NUM] 
		--	END 
		--FROM REG_PROVIDER reg
		--JOIN DCConv_KeyCrossReferences map ON reg.REG_ID = map.RegistrationID 
		--JOIN SRC_ProviderTaxIds tax ON tax.[P-SYS-ID] = map.SysID 
		--	AND dbo.fn_ConvertDCDateToPDMS(tax.[P-TAX-END-DT]) > @pin_conv_data_export_date  
		--JOIN SRC_Providers a ON a.[P-SYS-ID] = map.SysID
		--WHERE map.IsGroup = 1 AND tax.[EnableConversion] = 1
		--AND ISNULL(tax.[P-FED-TAX-ID],'') NOT IN ('','000000000');

		-- obsolete	
		---- If missing TAX_ID because of date range filter, get latest TAX_ID from source
		--WITH addTax as (
		-- SELECT tax.[P-SYS-ID], tax.[P-FED-TAX-ID]
		-- FROM SRC_ProviderTaxIds tax
		-- JOIN (SELECT [P-SYS-ID], MAX([P-TAX-END-DT]) as  [P-TAX-END-DT]
		-- 	   FROM SRC_ProviderTaxIds 
		--	   WHERE [P-FED-TAX-ID] NOT IN ('','000000000')
		--	   AND [EnableConversion] = 1
		--	   GROUP BY [P-SYS-ID]) sub
		...
		--FROM REG_PROVIDER reg
		--JOIN DCConv_KeyCrossReferences map ON reg.REG_ID = map.RegistrationID 
		--JOIN addTax tax ON tax.[P-SYS-ID] = map.SysID 
		--JOIN SRC_Providers a ON a.[P-SYS-ID] = map.SysID
		--WHERE map.IsGroup = 1 
		--AND ISNULL(reg.TAX_ID,'') IN ('','000000000');
		...
		SET [CONTACT_NAME] = primary_adr.[P-CONTCT-NAM] ,
			...
			[CONTACT_EMAIL_ADDRESS] = primary_adr.[P-CONTCT-EMAIL-AD-TEXT],
		...
		INSERT INTO [dbo].[REG_SERVICE_LOCATION]
		 ...
		 SELECT map.RegistrationId AS [REG_ID],
			   ...
			   remit_adr.[P-LINE1-AD] AS [REMITTANCE_ADDRESS1],
			   remit_adr.[P-LINE2-AD] AS [REMITTANCE_ADDRESS2],
			   remit_adr.[P-CITY-NAM] AS [REMITTANCE_CITY], 
			   --remit_adr.[P-CNTY-CD] AS [REMITTANCE_COUNTY],
			   remit_adr.[P-ST-CD] AS [REMITTANCE_STATE],
			   remit_adr.[P-ZIP5-CD] AS [REMITTANCE_ZIP],
			   remit_adr.[P-ZIP4-CD] AS [REMITTANCE_EXT_ZIP],
			   remit_adr.[P-CONTCT-PHON-NUM] AS [REMITTANCE_PHONE_NUMBER],
			   remit_adr.[P-CONTCT-NAM] AS [REMITTANCE_CONTACT_NAME],
			   CASE remit_adr.[P-NAM-ORG-IND]	WHEN 'N' THEN 'Individual' WHEN 'Y' THEN 'Organization' ELSE NULL END as [REMITTANCE_CONTACT_TYPE],
			   RTRIM(remit_adr.[P-CONTCT-EMAIL-AD-TEXT]) AS [REMITTANCE_EMAIL],
			   remit_adr.[P-FAX-NUM] AS [REMITTANCE_FAX_NUMBER],
			   CASE remit_adr.[P-NAM-ORG-IND] WHEN 'Y' THEN remit_adr.[P-NAM] ELSE NULL END as [REMITTANCE_ORGANIZATION_NAME],			   
			   remit_adr.[P-PHON-NUM] AS [REMITTANCE_ADDRESS_PHONE_NUMBER], 			   
			   CASE remit_adr.[P-NAM-ORG-IND] WHEN 'N' THEN remit_adr.[P-FST-NAM] ELSE NULL END as [REMITTANCE_ADDRESS_FIRSTNAME],
			   CASE remit_adr.[P-NAM-ORG-IND] WHEN 'N' THEN remit_adr.[P-MI-NAM] ELSE NULL END as [REMITTANCE_ADDRESS_MIDDLENAME],
			   CASE remit_adr.[P-NAM-ORG-IND] WHEN 'N' THEN remit_adr.[P-LAST-NAM] ELSE NULL END as [REMITTANCE_ADDRESS_LASTNAME],
			   CASE remit_adr.[P-NAM-ORG-IND] WHEN 'N' THEN remit_adr.[P-SFX-NAM] ELSE NULL END as [REMITTANCE_ADDRESS_TITLE],			   			   
			   oth_adr.[P-LINE1-AD] AS [OTHER_ADDRESS1],
			   oth_adr.[P-LINE2-AD] AS [OTHER_ADDRESS2],
			   oth_adr.[P-CITY-NAM] AS [OTHER_CITY], 
			   --oth_adr.[P-CNTY-CD] AS [OTHER_COUNTY],
			   oth_adr.[P-ST-CD] AS [OTHER_STATE],
			   oth_adr.[P-ZIP5-CD] AS [OTHER_ZIP],
			   oth_adr.[P-ZIP4-CD] AS [OTHER_EXT_ZIP],
			   oth_adr.[P-CONTCT-PHON-NUM] AS [OTHER_PHONE_NUMBER],
			   oth_adr.[P-CONTCT-NAM] AS [OTHER_CONTACT_NAME],
			   CASE oth_adr.[P-NAM-ORG-IND]	WHEN 'N' THEN 'Individual' WHEN 'Y' THEN 'Organization' ELSE NULL END as [OTHER_CONTACT_TYPE],
			   RTRIM(oth_adr.[P-CONTCT-EMAIL-AD-TEXT]) AS [OTHER_EMAIL],
			   oth_adr.[P-FAX-NUM] AS [OTHER_FAX_NUMBER],	
			   CASE oth_adr.[P-NAM-ORG-IND] WHEN 'Y' THEN oth_adr.[P-NAM] ELSE NULL END as [OTHER_ORGANIZATION_NAME],			   
			   oth_adr.[P-PHON-NUM] AS [OTHER_ADDRESS_PHONE_NUMBER],  			   
			   CASE oth_adr.[P-NAM-ORG-IND] WHEN 'N' THEN oth_adr.[P-FST-NAM] ELSE NULL END as [OTHER_ADDRESS_FIRSTNAME],
			   CASE oth_adr.[P-NAM-ORG-IND] WHEN 'N' THEN oth_adr.[P-MI-NAM] ELSE NULL END as [OTHER_ADDRESS_MIDDLENAME],
			   CASE oth_adr.[P-NAM-ORG-IND] WHEN 'N' THEN oth_adr.[P-LAST-NAM] ELSE NULL END as [OTHER_ADDRESS_LASTNAME],
			   CASE oth_adr.[P-NAM-ORG-IND] WHEN 'N' THEN oth_adr.[P-SFX-NAM] ELSE NULL END as [OTHER_ADDRESS_TITLE],			   			   
			   ...
			   CASE dbo.fn_GetProviderFiscalYearEnd(map.[SysID], prov.[P-MCARE-FY-MO-NUM],prov.[P-MCAID-FY-MO-NUM],prov.[P-FACI-FY-MO-NUM])
				...
				, CASE WHEN serv_adr.[G-QUAD-CD] in ('NW','NE','SE','SW') THEN serv_adr.[G-QUAD-CD] ELSE NULL END as [SERVICING_QUAD]
				...
				, CASE WHEN mail_adr.[G-QUAD-CD] in ('NW','NE','SE','SW') THEN mail_adr.[G-QUAD-CD] ELSE NULL END as [MAILTO_QUAD] 
				...
				, CASE WHEN pay_adr.[G-QUAD-CD] in ('NW','NE','SE','SW') THEN pay_adr.[G-QUAD-CD] ELSE NULL END as [PAYTO_QUAD]
				...
				, CASE WHEN remit_adr.[G-QUAD-CD] in ('NW','NE','SE','SW') THEN remit_adr.[G-QUAD-CD] ELSE NULL END as [REMITTANCE_QUAD]
				, CASE WHEN SUBSTRING(remit_adr.[G-WARD-CD],1,1) = '0' THEN SUBSTRING(remit_adr.[G-WARD-CD],2,1) 
					   WHEN remit_adr.[G-WARD-CD] = '' THEN NULL
					   ELSE remit_adr.[G-WARD-CD] END AS [REMITTANCE_WARD]			 
				, CASE WHEN oth_adr.[G-QUAD-CD] in ('NW','NE','SE','SW') THEN oth_adr.[G-QUAD-CD] ELSE NULL END as [OTHER_QUAD]
				, CASE WHEN SUBSTRING(oth_adr.[G-WARD-CD],1,1) = '0' THEN SUBSTRING(oth_adr.[G-WARD-CD],2,1) 
					   WHEN oth_adr.[G-WARD-CD] = '' THEN NULL
					   ELSE oth_adr.[G-WARD-CD] END AS [OTHER_WARD]						   
		...
		LEFT OUTER JOIN SRC_ProviderAddresses remit_adr ON remit_adr.[P-SYS-ID] = map.[SysID] AND remit_adr.[P-ADR-TY-CD] = 'R' AND remit_adr.[EnableConversion] = 1
		LEFT OUTER JOIN SRC_ProviderAddresses oth_adr ON oth_adr.[P-SYS-ID] = map.[SysID] AND oth_adr.[P-ADR-TY-CD] = 'O' AND oth_adr.[EnableConversion] = 1
		...
		WHERE map.IsGroup = 1 AND pq.[EnableConversion] = 1;
		...
		WHERE map.IsGroup = 1 AND bed.[EnableConversion] = 1 AND bed.[P-TOT-BED-NUM] > 0;
