...
/****** Object:  StoredProcedure [dbo].[DCConv_Validate]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_Validate]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_DropDCStagingTableSynonyms]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_DropDCStagingTableSynonyms]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_CreateUtilityTables]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_CreateUtilityTables]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_CreateDCStagingTableSynonyms]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_CreateDCStagingTableSynonyms]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertReferenceValues]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertReferenceValues]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertIndividuals_Step2]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertIndividuals_Step2]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertIndividuals_Step1]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertIndividuals_Step1]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertGroups_Step2]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertGroups_Step2]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertGroups_Step1]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertGroups_Step1]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertDCDataToPDMS]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertDCDataToPDMS]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ConvertAffiliations]    Script Date: 7/20/2016 5:54:07 PM ******/
DROP PROCEDURE [dbo].[DCConv_ConvertAffiliations]
GO

/****** Object:  StoredProcedure [dbo].[DCConv_ClearRegistrationData]    Script Date: 7/20/2016 5:54:07 PM ******/
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertAffiliations]    Script Date: 7/20/2016 5:54:09 PM ******/
SET ANSI_NULLS ON
GO
	...
	INSERT INTO [dbo].[REG_AFFILIATION]
     ...
     SELECT 
		-- get the registration ID of the most recent enrollment that occurred before the start date of the affiliation
			ISNULL((SELECT TOP 1 keyTable.[RegistrationId]
			FROM DCConv_KeyCrossReferences keyTable 
			WHERE keyTable.[SysID] = affil.[P-GROUP-SYS-ID] AND
			keyTable.[EnrollmentStartDate] = (SELECT TOP 1 keyTable.[EnrollmentStartDate] WHERE keyTable.[SysID] = affil.[P-GROUP-SYS-ID] AND keyTable.[EnrollmentStartDate] <= dbo.fn_ConvertDCDateToPDMS(affil.[P-AFFL-BEG-DT]) ORDER BY keyTable.[EnrollmentStartDate] DESC)),0)  AS [REG_ID], -- group reg id
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertDCDataToPDMS]    Script Date: 7/20/2016 5:54:10 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertGroups_Step1]    Script Date: 7/20/2016 5:54:10 PM ******/
SET ANSI_NULLS ON
GO
	...
	--	SELECT * FROM DCConv_KeyCrossReferences g1 WHERE (SELECT COUNT(*) FROM DCConv_KeyCrossReferences g2 WHERE g1.SysID=g2.SysID) > 0 ORDER BY RegistrationId;
		...
		INSERT INTO [dbo].[REG_PROVIDER]
			...
			SELECT DISTINCT map.RegistrationId AS [REG_ID],
				   ...
				   16 AS [TAX_ID_TYPE_ID], -- not sure what to do with this since the tax id table talks about being for provider in data elements, but the table description says it's for the owner.
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertGroups_Step2]    Script Date: 7/20/2016 5:54:12 PM ******/
SET ANSI_NULLS ON
GO
...
--		CASE WHEN ISDATE(en.[P-STAT-END-DT]) = 1 THEN CAST(en.[P-STAT-END-DT] AS datetime) ELSE '1/1/1753' END > @pin_conv_data_export_date​ AND
		...
		-- convert the taxonomy and specialty data
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertIndividuals_Step1]    Script Date: 7/20/2016 5:54:12 PM ******/
SET ANSI_NULLS ON
GO
...
--		BEGIN TRANSACTION
		...
		-- choose all providers with affiliates as group members
		INSERT INTO DCConv_KeyCrossReferences
		SELECT DISTINCT en.[P-SYS-ID], (ROW_NUMBER() OVER (ORDER BY en.[P-SYS-ID])) + @MaxRegistrationID,dbo.fn_ConvertDCDateToPDMS(en.[P-STAT-EFF-DT]), 0
		FROM SRC_Enrollments en
		INNER JOIN SRC_Providers prov ON prov.[P-SYS-ID] = en.[P-SYS-ID]
		WHERE prov.[P-REC-TY-CD] = 'P' AND -- en.[P-ENROL-STAT-TY-CD] = '00' AND 
--		en.[P-STAT-END-DT] IS NOT NULL AND
--		CASE WHEN ISDATE(en.[P-STAT-END-DT]) = 1 THEN CAST(en.[P-STAT-END-DT] AS datetime) ELSE '1/1/1753' END > @pin_conv_data_export_date​ AND
		...
		en.[P-TY-CD] NOT IN ('U04', 'U05', 'U07', 'U08')
		 ORDER BY en.[P-SYS-ID];
		--(SELECT COUNT(*) FROM SRC_ProviderAffiliates aff 
		--WHERE aff.[P-GROUP-SYS-ID] = en.[P-SYS-ID] AND 
		--aff.[P-MEMBER-SYS-ID] IS NOT NULL) = 0
		...
		INSERT INTO [dbo].[REG_PROVIDER]
			...
			SELECT DISTINCT map.RegistrationId AS [REG_ID],
				   ...
				   @pin_conv_run_time AS [LAST_MODIFIED_DATE_TIME],
				   dbo.fn_GetUniqueGUID(2) AS [LAST_MODIFIED_USER],
				   dbo.fn_ConvertEnrollmentStatusToPDMS((SELECT TOP 1 enr.[P-ENROL-STAT-TY-CD] FROM SRC_Enrollments enr WHERE dbo.fn_ConvertDCDateToPDMS(enr.[P-STAT-END-DT]) > @pin_conv_data_export_date  AND enr.[P-SYS-ID] = map.SysID ORDER BY dbo.fn_ConvertDCDateToPDMS(enr.[P-STAT-EFF-DT]) DESC)) AS ENROLLMENT_STATUS_CODE, -- use the most recent enrollment status that is current
				   NULL AS TERM_DATE, -- will have to change later
				   CASE WHEN LEN(RTRIM(LTRIM(prov.[P-FST-NAM]))) = 0 THEN prov.[P-DBA-FST-NAM] ELSE prov.[P-FST-NAM] END AS [FIRST_NAME],
				   CASE WHEN LEN(RTRIM(LTRIM(prov.[P-LAST-NAM]))) = 0 THEN prov.[P-DBA-LAST-NAM] ELSE prov.[P-LAST-NAM] END AS [LAST_NAME],
				   CASE WHEN LEN(RTRIM(LTRIM(prov.[P-MI-NAM]))) = 0 THEN prov.[P-DBA-MI-NAM] ELSE prov.[P-MI-NAM] END AS [MIDDLE_INITIAL],
				   ...
				   dbo.fn_ConvertTerminationReasonToPDMS((SELECT TOP 1 enr.[P-ENROL-STAT-TY-CD] FROM SRC_Enrollments enr WHERE [P-ENROL-STAT-TY-CD] <> '00' AND enr.[P-SYS-ID] = map.SysID ORDER BY dbo.fn_ConvertDCDateToPDMS(enr.[P-STAT-END-DT]) DESC)) AS [TERM_REASON_ID],
				   NULL AS [TYPE_OF_PRACTICE_ID], -- ** Must ask Jon what this should be
				   15 AS [TAX_ID_TYPE_ID], -- not sure what to do with this since the tax id table talks about being for provider in data elements, but the table description says it's for the owner.
				   (SELECT MIN(dbo.fn_ConvertDCDateToPDMS(alt_id.[P-ALT-ID-BEG-DT])) FROM SRC_ProviderAltIDs alt_id WHERE alt_id.[P-SYS-ID] = map.SysID AND alt_id.[P-ALT-ID-TY-CD] = 'XX' AND alt_id.[P-ALT-ID] = prov.[P-NPI-NUM] AND dbo.fn_ConvertDCDateToPDMS(alt_id.[P-ALT-ID-END-DT]) > @pin_conv_data_export_date)  AS [NPI_START_DATE],
				   (SELECT MAX(dbo.fn_ConvertDCDateToPDMS(alt_id.[P-ALT-ID-END-DT])) FROM SRC_ProviderAltIDs alt_id WHERE alt_id.[P-SYS-ID] = map.SysID AND alt_id.[P-ALT-ID-TY-CD] = 'XX' AND alt_id.[P-ALT-ID] = prov.[P-NPI-NUM] AND dbo.fn_ConvertDCDateToPDMS(alt_id.[P-ALT-ID-END-DT]) > @pin_conv_data_export_date) AS [NPI_END_DATE],
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertIndividuals_Step2]    Script Date: 7/20/2016 5:54:14 PM ******/
SET ANSI_NULLS ON
GO
...
--		CASE WHEN ISDATE(en.[P-STAT-END-DT]) = 1 THEN CAST(en.[P-STAT-END-DT] AS datetime) ELSE '1/1/1753' END > @pin_conv_data_export_date​ AND
		...
		INSERT INTO [dbo].[REG_PROVIDER]
			...
			FROM DCConv_KeyCrossReferences map
			INNER JOIN SRC_Providers prov ON prov.[P-SYS-ID] = map.SysID
			INNER JOIN SRC_ProviderAddresses primary_adr ON primary_adr.[P-SYS-ID] = map.SysID AND primary_adr.[P-ADR-TY-CD] = 'L' AND dbo.fn_ConvertDCDateToPDMS(primary_adr.[G-AUD-DT]) = (SELECT MAX(dbo.fn_ConvertDCDateToPDMS([G-AUD-DT])) FROM SRC_ProviderAddresses WHERE [P-SYS-ID] = map.SysID AND [P-ADR-TY-CD] = 'L')
			LEFT OUTER JOIN SRC_ProviderTaxIds tax ON tax.[P-SYS-ID] = map.SysID AND dbo.fn_ConvertDCDateToPDMS(tax.[P-TAX-END-DT]) > @pin_conv_data_export_date
			WHERE map.IsGroup = 0;
		...
		INNER JOIN REG_PROVIDER prov ON prov.[REG_ID] = map.[RegistrationID]
		WHERE map.IsGroup = 0 AND
		(SELECT TAXONOMY_TYPE_ID FROM dbo.TAXONOMY_TYPE WHERE RTRIM(LTRIM(UPPER(TAXONOMY_CODE))) = RTRIM(LTRIM(UPPER(tax.[P-TAXONOMY-CD]))) AND PROVIDER_TYPE_ID = prov.[PROVIDER_TYPE_ID]) > 0;
		...
		INNER JOIN REG_PROVIDER prov ON prov.[REG_ID] = map.[RegistrationID]
		WHERE map.IsGroup = 0 AND
		(SELECT SPECIALTY_TYPE_ID FROM dbo.TAXONOMY_TYPE WHERE RTRIM(LTRIM(UPPER(MMIS_SPECIALTY_TYPE_ID))) = RTRIM(LTRIM(UPPER(specl.[P-SPECL-CD]))) AND PROVIDER_TYPE_ID = prov.PROVIDER_TYPE_ID) > 0;
...
/****** Object:  StoredProcedure [dbo].[DCConv_ConvertReferenceValues]    Script Date: 7/20/2016 5:54:15 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[DCConv_CreateDCStagingTableSynonyms]    Script Date: 7/20/2016 5:54:16 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	CREATE SYNONYM SRC_Providers FOR  [DC_PDMS_CONVSRC02].[dbo].[DT_PROVDRTB_STG];
	CREATE SYNONYM SRC_Enrollments FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PENROLTB_STG];
	CREATE SYNONYM SRC_ProviderAddresses FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PADDRSTB_STG];
	CREATE SYNONYM SRC_ProviderAltIDs FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PALTIDTB_STG];
	CREATE SYNONYM SRC_ProviderTaxonomy FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PTAXONTB_STG];
	CREATE SYNONYM SRC_ProviderSpecialty FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PSPECLTB_STG];
	CREATE SYNONYM SRC_ProviderLicense FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PLICNSTB_STG];
	CREATE SYNONYM SRC_ProviderCLIA FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PCLIAPTB_STG];
	CREATE SYNONYM SRC_ProviderCLIADetails FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PCLIACTB_STG];
	CREATE SYNONYM SRC_ProviderMedicare FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PMCARETB_STG];
	CREATE SYNONYM SRC_ProviderMedicaid FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PREVMCTB_STG];
	CREATE SYNONYM SRC_ProviderOwner FOR [DC_PDMS_CONVSRC02].[dbo].[DT_POWNINTB_STG];
	CREATE SYNONYM SRC_ProviderTaxonomyHeader FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PTXNHDTB_STG];
	CREATE SYNONYM SRC_ProviderAffiliates FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PAFFILTB_STG];
	CREATE SYNONYM SRC_ProviderTaxIds FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PTAXIDTB_STG];
	CREATE SYNONYM SRC_ProviderNumOfBeds FOR [DC_PDMS_CONVSRC02].[dbo].[DT_PNUBEDTB_STG];
	
	CREATE SYNONYM SRC_LkUpAddressType FOR [DC_PDMS_CONVSRC02].[dbo].[LKUP_PROVIDER_ADDRESS_TYPE];
	CREATE SYNONYM SRC_LkUpProviderType FOR [DC_PDMS_CONVSRC02].[dbo].[LKUP_PROVIDER_TYPE_CODE];
	CREATE SYNONYM SRC_LkUpSpecialtyType FOR [DC_PDMS_CONVSRC02].[dbo].[LKUP_PROVIDER_SPECIALTY_CODE];
...
/****** Object:  StoredProcedure [dbo].[DCConv_CreateUtilityTables]    Script Date: 7/20/2016 5:54:17 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[DCConv_DropDCStagingTableSynonyms]    Script Date: 7/20/2016 5:54:18 PM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  StoredProcedure [dbo].[DCConv_Validate]    Script Date: 7/20/2016 5:54:19 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	INSERT INTO #Results 
	SELECT DISTINCT 'PROVIDER TYPE IGNORED' AS Validation_Type, dt.[P-SYS-ID] AS Provider_Sys_ID, prov.[P-ID] AS Provider_ID, '''' AS Group_Provider_ID, '''' AS Member_Provider_ID, '' AS Table_Name, '[P-TY-ID]' AS Column_Name, dt.[P-TY-CD] AS Bad_Value 
	FROM SRC_Enrollments dt
	INNER JOIN SRC_Providers prov ON prov.[P-SYS-ID] = dt.[P-SYS-ID]
	WHERE [P-TY-CD] IN ('U04', 'U05', 'U07', 'U08');

	INSERT INTO #Results 
	SELECT DISTINCT 'BAD TAXONOMY' AS Validation_Type, dt.[P-SYS-ID] AS Provider_Sys_ID, prov.[P-ID] AS Provider_ID, '''' AS Group_Provider_ID, '''' AS Member_Provider_ID, '' AS Table_Name, '[P-TAXONOMY-CD]' AS Column_Name, tax.[P-TAXONOMY-CD] AS Bad_Value 
	FROM SRC_Enrollments dt
	INNER JOIN SRC_Providers prov ON prov.[P-SYS-ID] = dt.[P-SYS-ID]
	INNER JOIN SRC_ProviderTaxonomy tax ON tax.[P-SYS-ID] = dt.[P-SYS-ID]
	WHERE 
	dbo.fn_ConvertProviderTaxonomyCodeToPDMS(tax.[P-TAXONOMY-CD], dt.[P-SYS-ID],@pin_conv_data_export_date) = 0;

	INSERT INTO #Results 
	SELECT DISTINCT 'BAD SPECIALTY' AS Validation_Type, dt.[P-SYS-ID] AS Provider_Sys_ID, prov.[P-ID] AS Provider_ID, '''' AS Group_Provider_ID, '''' AS Member_Provider_ID, '' AS Table_Name, '[P-SPECL-CD]' AS Column_Name, tax.[P-SPECL-CD] AS Bad_Value 
	FROM SRC_Enrollments dt
	INNER JOIN SRC_Providers prov ON prov.[P-SYS-ID] = dt.[P-SYS-ID]
	INNER JOIN SRC_ProviderSpecialty tax ON tax.[P-SYS-ID] = dt.[P-SYS-ID]
	WHERE 
	dbo.fn_ConvertDCSpecialtyTypeToPDMS(tax.[P-SPECL-CD],dt.[P-SYS-ID], @pin_conv_data_export_date) = 0;
